...
import com.market.order.service.ReviewService;
...
public class ReviewController {
     ...
     */
    @GetMapping("/check")
    public Result<Boolean> checkUserReviewedItem(@RequestParam Integer itemId, @RequestParam Integer userId) {
        if (itemId == null || userId == null) {
            return Result.fail(400, "缺少必要的参数");
        }
        int count = reviewService.getReviewMapper().checkUserReviewedItem(itemId, userId);
    ...
    public Result<Review> createReview(@RequestBody Review review) {
        ...
        // 检查用户是否已评价过该商品
        int existingReview = reviewService.getReviewMapper().checkUserReviewedItem(review.getItemId(), review.getUserId());
        if (existingReview > 0) {
            return Result.fail(409, "您已经评价过该商品，不能重复评价");
        }
